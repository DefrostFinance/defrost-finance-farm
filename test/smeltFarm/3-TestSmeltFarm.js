...
//set phxfarm///////////////////////////////////////////////////////////
  ...
  let smeltaddr = await farminst.smelt();
  smelt = await Smelt.at(smeltaddr);

  let block = await web3.eth.getBlock("latest");
  startTime = block.timestamp + 3600*24;
...
//////////////////////////////////////////////////////////////////////////////////////////
  ...
  {
      //use h2o mine rate to decide when to start mine
      let msgData = farminst.contract.methods.setMineRate(web3.utils.toWei(""+0,"ether"),day).encodeABI();
      let hash = await utils.createApplication(mulSiginst, operator0, farminst.address, 0, msgData);
      let index = await mulSiginst.getApplicationCount(hash);
      index = index.toNumber() - 1;
      console.log(index);
  ...
  //set mine Rate
  res = await farminst.setMineRate(web3.utils.toWei(""+0,"ether"),day,{from:operator0});
  assert.equal(res.receipt.status,true);
  ...
  it("[0010] stake in before farm begin time,should pass", async()=>{
  ...
  it("[0020] check staker1 mined balance,should pass", async()=>{
    ...
    let meltpreBalance = web3.utils.fromWei(await h2o.balanceOf(staker1));
     ...
     res = await farminst.allPendingReward(staker1)
     console.log("staker1 melt number",web3.utils.fromWei(res[0].toString()),"melt mine pending should be 0",res[1].toString());

  })

  it("[0030] check smelt transfer before farm,should pass", async()=>{

        res = await smelt.transfer(staker3,web3.utils.toWei("100000"),{from:staker1});
        time.increase(3600);//2000 sec
...
///////////////////////////////////staker1/////////////////////////////////////////
        ...
        console.log("staker1 intterest",res[0].toString(),"melt mine pending",res[1].toString());
///////////////////////////////////////////////////////////////////////////////////////
        res = await farminst.allPendingReward(staker3)
        console.log("staker3 intterest",res[0].toString(),"melt mine pending",res[1].toString());


        meltpreBalance = web3.utils.fromWei(await h2o.balanceOf(staker3));

        res = await farminst.withdraw(0,{from:staker3});
        assert.equal(res.receipt.status,true);

        meltafterBalance = web3.utils.fromWei(await h2o.balanceOf(staker3))
        console.log("staker3 h2o reward=" + (meltafterBalance - meltpreBalance));

        res = await farminst.allPendingReward(staker3)
        console.log("staker3 intterest",res[0].toString(),"melt mine pending",res[1].toString());
    })

    it("[0040] stake out before farm start time,should pass", async()=>{
        console.log("\n\n");
  ...
  it("[0050] set h2o reward again,should pass", async()=> {
      {
          let msgData = farminst.contract.methods.setMineRate(web3.utils.toWei("" + 3600, "ether"), day).encodeABI();
          let hash = await utils.createApplication(mulSiginst, operator0, farminst.address, 0, msgData);
          let index = await mulSiginst.getApplicationCount(hash);
          index = index.toNumber() - 1;
          console.log(index);
      ...
      //set mine Rate
      res = await farminst.setMineRate(web3.utils.toWei("" + 3600, "ether"), day, {from: operator0});
      assert.equal(res.receipt.status, true);
  })

  it("[0060] stake in after farm begin time and reset mine rate,should pass", async()=>{
    ...
    it("[0070] check staker1 mined balance,should pass", async()=>{
        time.increase(7200);//2000 sec
        let res = await farminst.totalStaked();
        console.log("totalstaked=" + res);
   ...
   it("[0080] check smelt transfer and withdraw reward again,should pass", async()=>{
